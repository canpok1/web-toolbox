...
test.describe('セッション参加画面', () => {
    ...
    test('「セッションに参加」ボタンがあること', async ({ page }) => {
        await expect(page.getByRole('button', { name: 'セッションに参加' })).toBeVisible();
    });
    ...
    test('セッションIDと名前が入力されていないときはセッション参加ボタンが押せないこと', async ({ page }) => {
        const joinSessionButton = page.getByRole('button', { name: 'セッションに参加' });
        await expect(joinSessionButton).toBeDisabled();
    });

    test('セッションIDと名前が入力されたときはセッション参加ボタンが押せること', async ({ page }) => {
    ...
    test('存在しないセッションIDを入力してセッション参加ボタンを押すと画面遷移せずにエラーが表示されること', async ({ page }) => {
